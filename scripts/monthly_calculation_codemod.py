...
MONTHLY_CALCULATION_PATTERNS = {
    ...
    "base_hourly_formula": [
        ...
        (
            r"base_salary\s*/\s*Decimal\(['\"]180['\"]\)",
            "base_salary / MONTHLY_NORM_HOURS",
        ),
        (r"monthly_salary\s*/\s*180", "monthly_salary / 182"),
        (
            r"monthly_salary\s*/\s*Decimal\(['\"]180['\"]\)",
            "monthly_salary / MONTHLY_NORM_HOURS",
        ),
        # Standardize hourly rate variable names
        (
            r"hourly_rate\s*=\s*base_salary\s*/\s*\d+",
            "monthly_hourly = base_salary / MONTHLY_NORM_HOURS",
        ),
        (
            r"hour_rate\s*=\s*base_salary\s*/\s*\d+",
            "monthly_hourly = base_salary / MONTHLY_NORM_HOURS",
        ),
    ],
    ...
    "proportional_calculation": [
        (
            r"proportional\s*=\s*hourly_rate\s*\*\s*worked_hours",
            "proportional = monthly_hourly * worked_hours",
        ),
        (
            r"base_pay\s*=\s*hourly_rate\s*\*\s*regular_hours",
            "proportional = monthly_hourly * worked_hours",
        ),
        (
            r"regular_pay\s*=\s*hourly_rate\s*\*\s*hours",
            "proportional = monthly_hourly * worked_hours",
        ),
    ],
    ...
    "night_weekday_bonuses": [
        # 0-7 hours: no bonus
        (
            r"night.*hours\s*<=\s*7.*\*\s*1\.0",
            "# Night 0-7 hours: no bonus (base rate only)",
        ),
        # 7-9 hours: 25% bonus
        (
...
def apply_monthly_calculation_standardizations(content: str) -> Tuple[str, int]:
    ...
    for category, patterns in MONTHLY_CALCULATION_PATTERNS.items():
        for old_pattern, new_pattern in patterns:
            ...
            if not re.search(
                r"(monthly|salary|hourly|proportional|bonus)",
                modified_content,
                re.IGNORECASE,
            ):
                continue
            ...
            matches = re.findall(
                old_pattern, modified_content, re.IGNORECASE | re.MULTILINE
            )
            if matches:
                ...
                print(
                    f"    {category}: {len(matches)} replacements ({old_pattern[:50]}... -> {new_pattern[:50]}...)"
                )
...
def process_test_file(file_path: str, dry_run: bool = True) -> Tuple[bool, int]:
    ...
    modified_content, replacements_made = apply_monthly_calculation_standardizations(
        original_content
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Standardize monthly payroll calculation formulas"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
