...
CTOR_CALLS = [
    (
        r"(\s*)service\s*=\s*EnhancedPayrollCalculationService\(([^)]+)\)",
        "service_replacement",
    ),
...
def extract_params_from_constructor(params_str: str) -> dict:
    ...
    pattern = (
        r"(\w+)\s*,\s*(\d+|\w+)\s*,\s*(\d+|\w+)(?:\s*,\s*fast_mode\s*=\s*(True|False))?"
    )
...
def replace_service_instantiation(text: str) -> str:
    ...
    for pattern, replacement_type in CTOR_CALLS:
        ...
        def replacer(match):
            ...
            fast_mode_arg = (
                f", fast_mode={params['fast_mode']}" if params["fast_mode"] else ""
            )
    ...
    for i, line in enumerate(lines):
        ...
        if re.search(r"\bservice\.\w+", line) and "service =" not in line:
            ...
            if not service_defined and "self.assertEqual(service" in line:
                ...
                result.append(
                    f"        # TODO: Fix service reference - {line.strip()}\n"
                )
                continue
...
def fix_pytest_fixtures(text: str) -> str:
    ...
    if (
        "class.*TestCase" in text
        and "self.payroll_service = PayrollService()" not in text
    ):
            ...
            if "def setUp(self):" in line:
                ...
                for j in range(i + 1, min(i + 10, len(lines))):
                    if lines[j].strip() and not lines[j].strip().startswith(
                        ...
                        lines.insert(
                            j, "        self.payroll_service = PayrollService()\n"
                        )
                        break
                break
        text = "".join(lines)
...
def main():
    parser = argparse.ArgumentParser(
        description="Wave C: Migrate tests to PayrollService"
    )
    parser.add_argument(
    ...
    print(f"\n{'='*60}")
    print(
        f"Summary: {changed_count} files {'updated' if args.apply else 'would be updated'}"
    )
    if not args.apply:
