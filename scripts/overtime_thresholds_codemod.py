    ...
    for category, patterns in OVERTIME_STANDARDIZATIONS.items():
        for old_pattern, new_pattern in patterns:
            ...
            if category.endswith('_8_0') and old_pattern == r"8\.0":
                ...
                if re.search(r"(hours?|overtime|daily.*norm)", modified_content, re.IGNORECASE):
            ...
            if matches:
                ...
                print(f"    {category}: {len(matches)} replacements ({old_pattern} -> {new_pattern})")
...
def process_test_file(file_path: str, dry_run: bool = True) -> Tuple[bool, int]:
    ...
    modified_content, replacements_made = apply_overtime_standardizations(original_content)
...
def main():
    parser = argparse.ArgumentParser(description='Standardize overtime thresholds and rates')
