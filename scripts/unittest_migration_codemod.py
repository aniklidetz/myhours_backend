...
UNITTEST_MIGRATION_PATTERNS = {
    ...
    "remove_fixture_params": [
        ...
        (r"def test_\w+\(self, payroll_service\):", r"def test_\w+(self):"),
        (
            r"def test_\w+\(self, payroll_service, ([^)]+)\):",
            r"def test_\w+(self, \1):",
        ),
        # Remove israeli_labor_constants fixture parameter
        (r"def test_\w+\(self, israeli_labor_constants\):", r"def test_\w+(self):"),
        (
    ...
    "add_mixin": [
        ...
        (r"class (\w+Test.*)\(TestCase\):", r"class \1(PayrollTestMixin, TestCase):"),
        (
    ...
    "fixture_to_setup": [
        ...
        (r"payroll_service\.calculate\(", "self.payroll_service.calculate("),
        (r"payroll_service\.", "self.payroll_service."),
        # Replace constants fixture usage
        (r"israeli_labor_constants\[(['\"])([^'\"]+)\1\]", r"self.constants['\2']"),
    ],
...
def find_test_files(base_path: str) -> List[str]:
    ...
    for test_file in payroll_path.rglob("test_*.py"):
        ...
        if (
            "legacy" in str(test_file)
            or "archive" in str(test_file)
            or test_file.name == "helpers.py"
        ):
            continue
...
def analyze_test_file(content: str) -> Dict[str, bool]:
    ...
    analysis = {
        "has_conftest_imports": bool(
            re.search(r"from payroll\.tests\.conftest import", content)
        ),
        "has_fixture_params": bool(
            re.search(
                r"def test_\w+\([^)]*(?:payroll_service|israeli_labor_constants)",
                content,
            )
        ),
        "has_fixture_usage": bool(
            re.search(r"(?:payroll_service|israeli_labor_constants)\.", content)
        ),
        "needs_mixin": bool(
            re.search(r"payroll_service\.|israeli_labor_constants\[", content)
        ),
...
def apply_unittest_migrations(
    content: str, analysis: Dict[str, bool]
) -> Tuple[str, int]:
            ...
            for old_pattern, new_pattern in patterns:
                ...
                if matches:
                    modified_content = re.sub(
                        old_pattern, new_pattern, modified_content
                    )
...
def process_test_file(file_path: str, dry_run: bool = True) -> Tuple[bool, int]:
    ...
    modified_content, replacements_made = apply_unittest_migrations(
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate payroll tests from pytest to unittest patterns"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
