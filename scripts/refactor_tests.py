...
""",
            ...
            if content != original_content:
                ...
                self.changes_made.append(
                    f"Updated {test_file}: Replaced adapter patterns"
                )
    ...
    def wave_c_fix_labor_constants(self):
        ...
        for test_file in self.target_test_files:
            ...
            if "israeli_labor_constants" not in content and (
                "8.6" in content or "8.0" in content
            ):
            ...
            if content != original_content:
                if not self.dry_run:
                    file_path.write_text(content)
                self.changes_made.append(
                    f"Updated {test_file}: Fixed labor law constants"
                )
        ...
        for test_file in self.target_test_files:
            ...
            for line in lines:
                ...
                if line.strip() and not (
                    line.strip().startswith("from payroll.services.adapters")
                    or "EnhancedPayrollCalculationService" in line
                    and "import" in line
                ):
            ...
            if content != original_content:
                if not self.dry_run:
                    file_path.write_text(content)
                self.changes_made.append(
                    f"Updated {test_file}: Removed adapter dependencies"
                )
    ...
    def update_static_checker(self):
        ...
        if "adapters" not in content:
            ...
            pattern_match = re.search(
                r"LEGACY_PATTERNS = \[(.*?)\]", content, re.DOTALL
            )
            if pattern_match:
    ...
    re.compile(r'from\\s+payroll\\.services\\.adapters\\s+import'),"""
                ...
                content = content.replace(
                    pattern_match.group(0), f"LEGACY_PATTERNS = [{new_patterns}\n]"
                )
                ...
                self.changes_made.append(
                    "Updated static checker to catch adapter usage"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Refactor tests from adapters to PayrollService"
    )
    parser.add_argument(
        "--wave", choices=["A", "B", "C", "D"], help="Run specific wave"
    )
