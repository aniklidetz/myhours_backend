...
def fix_test_file_imports(file_path: str):
    ...
    if (
        "from payroll.tests.helpers import MONTHLY_NORM_HOURS\nfrom payroll.tests.helpers import"
        in content
    ):
        content = re.sub(
    ...
    if "PayrollTestMixin" in content and "from payroll.tests.helpers import" in content:
        if (
            "PayrollTestMixin"
            not in content.split("from payroll.tests.helpers import")[1].split("\n")[0]
        ):
            content = re.sub(
    ...
    content = re.sub(
        r"self\.self\.self\.payroll_service", "self.payroll_service", content
    )
