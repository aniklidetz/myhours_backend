...
def debug_auth(request):
    ...
    """
    auth_info = {
        'timestamp': str(request._start_time) if hasattr(request, '_start_time') else 'not_set',
    ...
    if hasattr(request, 'device_token'):
        ...
        auth_info['device_token'] = {
            'device_id': device_token.device_id[:8] + '...' if device_token.device_id else 'NOT_SET',
...
def debug_worktime_auth(request):
    ...
    if request.user.is_authenticated:
        ...
        if hasattr(request.user, 'employee_profile'):
            ...
            debug_info['permissions']['can_see_all'] = emp.role in ['accountant', 'admin'] or request.user.is_staff
        ...
        debug_info['recommendations'].append('User is not authenticated - check Authorization header')
        
        # Check if header exists but is malformed
        auth_header = request.META.get('HTTP_AUTHORIZATION', '')
        if auth_header:
            debug_info['recommendations'].append(f'Authorization header exists but authentication failed: {auth_header[:20]}...')
        else:
            debug_info['recommendations'].append('No Authorization header found')
    
    # Log for debugging
    logger.warning(f"Worktime Auth Debug: {debug_info}")
