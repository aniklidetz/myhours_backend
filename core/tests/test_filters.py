...
class WorkLogFilterTest(TestCase):
    ...
    def setUp(self):
        ...
        self.worklog_today = WorkLog.objects.create(
            ...
            check_in=timezone.make_aware(datetime.combine(self.today, datetime.min.time().replace(hour=9))),
            check_out=timezone.make_aware(datetime.combine(self.today, datetime.min.time().replace(hour=17)))
        )
        
        # Work log for yesterday
        self.worklog_yesterday = WorkLog.objects.create(
            employee=self.employee,
            check_in=timezone.make_aware(datetime.combine(self.yesterday, datetime.min.time().replace(hour=9))),
            check_out=timezone.make_aware(datetime.combine(self.yesterday, datetime.min.time().replace(hour=17)))
        )
        
        # Work log for tomorrow
        self.worklog_tomorrow = WorkLog.objects.create(
            employee=self.employee,
            check_in=timezone.make_aware(datetime.combine(self.tomorrow, datetime.min.time().replace(hour=9))),
            check_out=timezone.make_aware(datetime.combine(self.tomorrow, datetime.min.time().replace(hour=17)))
        )
    ...
    def test_multiple_worklogs_same_date(self):
        ...
        worklog_today_2 = WorkLog.objects.create(
            employee=self.employee,
            check_in=timezone.make_aware(datetime.combine(self.today, datetime.min.time().replace(hour=18))),
            check_out=timezone.make_aware(datetime.combine(self.today, datetime.min.time().replace(hour=22)))
        )
    ...
    def test_filter_with_datetime_object(self):
        ...
        today_datetime = timezone.make_aware(datetime.combine(self.today, datetime.min.time().replace(hour=12)))
        
        filter_set = WorkLogFilter(
            data={'date': today_datetime.date()},
            queryset=WorkLog.objects.all()
        )
    ...
    def test_filter_case_sensitivity(self):
        ...
        filter_set = WorkLogFilter(
            data={'DATE': self.today},  # Wrong case
            queryset=WorkLog.objects.all()
        )
    ...
    def test_worklog_across_midnight_filter(self):
        ...
        worklog_midnight = WorkLog.objects.create(
            employee=self.employee,
            check_in=timezone.make_aware(datetime.combine(self.today, datetime.min.time().replace(hour=23))),
            check_out=timezone.make_aware(datetime.combine(self.tomorrow, datetime.min.time().replace(hour=1)))
        )
