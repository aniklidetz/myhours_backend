...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['employee_id']:
            active_sessions = active_sessions.filter(employee__id=options['employee_id'])
            
        if options['employee_email']:
            active_sessions = active_sessions.filter(employee__email=options['employee_email'])
        
        if not active_sessions.exists():
            self.stdout.write(self.style.WARNING("No active sessions found for specified criteria"))
            return
        ...
        if not options['all']:
            ...
            confirm = input(f"Close {active_sessions.count()} active session(s)? [y/N]: ")
