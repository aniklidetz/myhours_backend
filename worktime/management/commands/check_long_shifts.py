...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--days", type=int, default=90, help="Number of days to check (default: 90)"
        )
        parser.add_argument(
            "--employee", type=int, help="Check specific employee by ID"
        )
        parser.add_argument(
            "--export", action="store_true", help="Export results to CSV file"
        )
        parser.add_argument(
            "--fix", action="store_true", help="Attempt to fix invalid shifts"
        )
        parser.add_argument(
            "--shorten",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        queryset = WorkLog.objects.filter(
            check_out__isnull=False, check_in__gte=start_date
        )
        ...
        long_shifts = (
            queryset.filter(duration__gt=timedelta(hours=12))
        ...
        if total_count == 0:
            self.stdout.write(self.style.SUCCESS("✅ No problematic shifts found!"))
            return
    ...
    def _check_overnight_shifts(self, long_shifts):
        ...
        for shift in long_shifts:
            ...
            if (
                check_in_hour >= 18 or check_in_hour <= 6
            ) and shift.duration < timedelta(hours=16):
    ...
    def _employee_statistics(self, long_shifts):
        ...
        # Sort by count
        sorted_stats = sorted(
            employee_stats.items(), key=lambda x: x[1]["count"], reverse=True
        )
    ...
    def _export_to_csv(self, long_shifts):
                ...
                writer.writerow(
                    {
                        ...
                        "crosses_midnight": shift.check_in.date()
                        != shift.check_out.date(),
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✅ Exported {long_shifts.count()} records to {filename}"
            )
        )
    ...
    def _fix_invalid_shifts(self, long_shifts):
        ...
        for shift in long_shifts:
            ...
            if duration_hours > 48:
                ...
                self.stdout.write(f"   Employee: {shift.employee.get_full_name()}")
                ...
                self.stdout.write(self.style.WARNING("   ⚠️ Fix not applied (dry run)"))
    ...
    def _shorten_long_shifts(self, long_shifts, dry_run=False):
        ...
        if dry_run:
            ...
            if shortened_count > 0:
                self.stdout.write(
                    self.style.SUCCESS(f"✅ All shifts are now 12 hours or less!")
                )
