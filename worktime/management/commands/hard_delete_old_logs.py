...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"🔍 Looking for soft-deleted work logs older than {days} days ({cutoff_date.strftime('%Y-%m-%d')})"
        )
        ...
        if count == 0:
            self.stdout.write(
                self.style.SUCCESS("✅ No old soft-deleted records found")
            )
            return
        ...
        self.stdout.write(
            self.style.ERROR(
                f"⚠️  WARNING: This will PERMANENTLY delete {count} work log records!"
            )
        )
        ...
        logger.warning(
            f"Hard deleted {deleted_count} old work log records older than {days} days"
        )
