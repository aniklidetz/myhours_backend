...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("🔍 Starting worktime N+1 query analysis...")
        )
        ...
        if employee_id:
            worklogs = WorkLog.objects.filter(employee_id=employee_id).order_by(
                "-check_in"
            )[:limit]
        else:
        ...
        if unoptimized_queries > len(data) + 1:
            self.stdout.write(
                self.style.ERROR(
                    f"  ⚠️  POTENTIAL N+1 DETECTED! Expected ~{len(data) + 1} queries, got {unoptimized_queries}"
                )
            )
        else:
        ...
        if employee_id:
            worklogs_optimized = (
                WorkLog.objects.select_related("employee")
                .filter(employee_id=employee_id)
                .order_by("-check_in")[:limit]
            )
        else:
            worklogs_optimized = WorkLog.objects.select_related("employee").order_by(
                "-check_in"
            )[:limit]
        ...
        if optimized_queries <= 2:  # Should be 1-2 queries max
            ...
            self.stdout.write(
                self.style.WARNING(f"  ⚠️  Still more queries than expected")
            )
        ...
        if serializer_queries <= 2:
            self.stdout.write(
                self.style.SUCCESS(f"  ✅ Serializer query count is optimal")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"  ⚠️  Serializer might have N+1 issues")
            )
        ...
        if improvement > 0:
            self.stdout.write(
                self.style.SUCCESS(
                    f"  🎉 Query reduction: {improvement} queries ({improvement/unoptimized_queries*100:.1f}% improvement)"
                )
            )
