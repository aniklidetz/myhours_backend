...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        active_sessions = WorkLog.objects.filter(check_out__isnull=True).select_related(
            "employee"
        )
        ...
        for emp_id, emp_data in sessions_by_employee.items():
            ...
            if options["detailed"]:
                for i, session in enumerate(sessions, 1):
                    ...
                    self.stdout.write(
                        f"      Location: {session.location_check_in or 'N/A'}"
                    )
                    ...
                    if duration > 12:
                        self.stdout.write(
                            self.style.WARNING(
                                f"      ⚠️  Long session detected ({duration} hours)"
                            )
                        )
            else:
                for session in sessions:
                    ...
                    self.stdout.write(
                        f"   - Started: {session.check_in} ({duration}h{warning})"
                    )
        ...
        self.stdout.write(
            f"   Total Employees with Active Sessions: {len(sessions_by_employee)}"
        )
        ...
        self.stdout.write(
            f"     python manage.py close_active_sessions --employee-email EMAIL"
        )
        ...
        if admin_sessions.exists() and not options["admin_only"]:
            ...
            self.stdout.write(
                f"     python manage.py list_active_sessions --admin-only --detailed"
            )
