...
class WorkLogAdmin(admin.ModelAdmin):
    list_display = ('employee', 'check_in', 'check_out', 'get_total_hours', 'get_status', 'soft_delete_info')
    ...
    def soft_delete_selected(self, request, queryset):
        ...
        for obj in queryset.filter(is_deleted=False):
            obj.soft_delete(deleted_by=request.user.employee_profile if hasattr(request.user, 'employee_profile') else None)
            count += 1
