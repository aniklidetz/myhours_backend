...
class BiometricVerificationRequired(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if device_token.biometric_verified_at:
            time_since_verification = timezone.now() - device_token.biometric_verified_at
...
class WorkTimeOperationPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        required_operations = getattr(settings, 'BIOMETRIC_VERIFICATION_REQUIRED_FOR', [])
...
class LocationBasedPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        location_data = request.data.get('location') if hasattr(request, 'data') else None
...
class EmployeeWithBiometric(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        biometric_check = BiometricVerificationRequired(max_age_hours=8).has_permission(request, view)
...
class AccountantWithBiometric(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        biometric_check = BiometricVerificationRequired(max_age_hours=1).has_permission(request, view)
