...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--user-id", type=int, help="Reset token for specific user ID"
        )
        parser.add_argument(
            "--username", type=str, help="Reset token for specific username"
        )
        parser.add_argument(
            "--all", action="store_true", help="Reset tokens for all users"
        )
    ...
    def handle(self, *args, **options):
        if options["user_id"]:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✅ Reset tokens for {users.count()} users")
            )

        else:
            self.stdout.write(
                self.style.ERROR("Please specify --user-id, --username, or --all")
            )
