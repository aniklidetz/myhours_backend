...
class DeviceTokenAuthenticationTest(TestCase):
    ...
    def test_authenticate_too_many_parts(self):
        ...
        request = self.factory.get(
            "/", HTTP_AUTHORIZATION="DeviceToken token1 token2 token3"
        )
        with self.assertRaises(AuthenticationFailed) as cm:
            self.auth.authenticate(request)
    ...
    def test_authenticate_unicode_error(self):
        ...
        with patch(
            "users.authentication.DeviceTokenAuthentication.get_authorization_header"
        ) as mock_get_header:
    ...
    def test_authenticate_credentials_expired_token(self):
        ...
        with self.assertRaises(AuthenticationFailed) as cm:
            self.auth.authenticate_credentials(
                "expired-token-123", self.factory.get("/")
            )
    ...
    def test_authenticate_credentials_inactive_user(self):
        ...
        with self.assertRaises(AuthenticationFailed) as cm:
            self.auth.authenticate_credentials(
                "test-token-abcdef123456", self.factory.get("/")
            )
    ...
    def test_authenticate_credentials_success(self):
        ...
        with patch(
            "users.authentication.DeviceTokenAuthentication.get_client_ip"
        ) as mock_get_ip:
            mock_get_ip.return_value = "192.168.1.1"

            user, token = self.auth.authenticate_credentials(
                "test-token-abcdef123456", request
            )
    ...
    def test_full_authentication_flow(self):
        ...
        request = self.factory.get(
            "/", HTTP_AUTHORIZATION="DeviceToken test-token-abcdef123456"
        )
...
class BiometricSessionAuthenticationTest(TestCase):
    ...
    def test_authenticate_no_device_token(self):
        ...
        with patch(
            "users.authentication.DeviceTokenAuthentication.authenticate"
        ) as mock_device_auth:
...
class HybridAuthenticationTest(TestCase):
    ...
    def test_authenticate_unicode_error(self):
        ...
        with patch(
            "users.authentication.HybridAuthentication.get_authorization_header"
        ) as mock_get_header:
            mock_get_header.return_value = (
                b"DeviceToken \xff\xfe"  # Return bytes, not list
            )
            request = self.factory.get("/")
...
class SecurityMiddlewareTest(TestCase):
    ...
    def test_is_ip_locked_out_expired_lockout(self):
        ...
        old_time = timezone.now() - timedelta(
            seconds=self.middleware.lockout_duration + 100
        )
...
class AuthenticationIntegrationTest(TestCase):
    ...
    def test_full_device_token_flow(self):
        ...
        request = self.factory.get(
            "/", HTTP_AUTHORIZATION="DeviceToken integration-test-token"
        )
    ...
    def test_biometric_session_complete_flow(self):
        ...
        request = self.factory.get(
            "/", HTTP_AUTHORIZATION="DeviceToken biometric-test-token"
        )
