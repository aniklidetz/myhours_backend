...
def enhanced_login(request):
    ...
    try:
        ...
        device_token = DeviceToken.create_token(
            user=user, device_id=device_id, device_info=device_info, ttl_days=ttl_days
        )
        ...
        requires_biometric = (
            biometric_registered and device_token.requires_biometric_verification()
        )

        # Log successful login
        logger.info(
            f"Enhanced login successful: user={user.username}, device={device_id[:8]}..."
        )
        ...
        return Response(
            {
                ...
                "security_info": {
                    ...
                    "last_biometric_verification": (
                        device_token.biometric_verified_at.isoformat()
...
def biometric_verification(request):
    ...
    try:
        ...
        verification_level = (
            ...
            else "medium" if confidence_score >= 0.7 else "low"
        )
        access_granted = {
            "payroll": verification_level in ["high", "medium"]
            and operation_type == "payroll",
            "admin_actions": verification_level == "high"
            and operation_type == "admin_actions",
        ...
        try:
            session_id = (
                str(biometric_session.session_id)
                if biometric_session and biometric_session.session_id
                else None
            )
            expires_at = (
                biometric_session.expires_at.isoformat()
                if biometric_session and biometric_session.expires_at
                else None
            )
        except Exception:
            session_id = None
...
def logout_device(request):
    ...
    try:
        ...
        logger.info(
            f"Device logout successful: user={request.user.username}, "
            f"device={device_token.device_id[:8]}..."
        )
