...
class DeviceToken(models.Model):
    ...
    def mark_used(self, ip_address=None, location=None):
        ...
        self.save(update_fields=['last_used', 'usage_count', 'last_ip', 'last_location'])
    ...
    def create_token(cls, user, device_id, device_info=None, ttl_days=7):
        ...
        try:
            ...
            device_token.save()
            logger.info(f"Updated existing device token for user {user.username}, device {device_id[:8]}...")
            ...
            logger.info(f"Created new device token for user {user.username}, device {device_id[:8]}...")
...
class BiometricSession(models.Model):
    ...
    @classmethod
    def create_session(cls, device_token, confidence_score, quality_score=None, 
                      location=None, ip_address=None, ttl_hours=8):
        """Create a new biometric session"""
        expires_at = timezone.now() + timedelta(hours=ttl_hours)
        
        # End any existing active sessions for this device
        cls.objects.filter(device_token=device_token, is_active=True).update(is_active=False)
