...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if len(all_embeddings) < 2:
            self.stdout.write("❌ Need at least 2 employees with embeddings for testing")
            return
        ...
        if emp1_embeddings and 'vector' in emp1_embeddings[0]:
            ...
            self.stdout.write("Tolerance | Match Emp1 | Distance Emp1 | Match Emp2 | Distance Emp2")
            ...
            for tolerance in tolerance_levels:
                ...
                emp1_known = [np.array(emb['vector']) for emb in emp1_embeddings if 'vector' in emb]
                match1, conf1 = face_processor.compare_faces(test_encoding, emp1_known)
                distance1 = 1 - conf1
                
                # Test against employee 2 (should not match)
                emp2_known = [np.array(emb['vector']) for emb in emp2_embeddings if 'vector' in emb]
                match2, conf2 = face_processor.compare_faces(test_encoding, emp2_known)
                distance2 = 1 - conf2
                
                self.stdout.write(f"{tolerance:>9.1f} | {match1:>10} | {distance1:>13.3f} | {match2:>10} | {distance2:>13.3f}")
            ...
            for employee_id, employee_embeddings in all_embeddings:
                ...
                if known_encodings:
                    is_match, confidence = face_processor.compare_faces(test_encoding, known_encodings)
            ...
            for emp_id, conf, match in all_matches:
                ...
                self.stdout.write(f"  Employee {emp_id}: confidence={conf:.3f} - {status}")
