...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--employee-id", type=int, help="Check specific employee ID only"
        )
    ...
    def _display_text_results(self, result):
        ...
        if result.get("employee_specific"):
            ...
            if pg.get("exists"):
                ...
                self.stdout.write(
                    f"  - Embeddings count: {pg.get('embeddings_count', 0)}"
                )
            ...
            self.stdout.write(
                f"📊 Total PostgreSQL profiles: {result.get('total_pg_profiles', 0)}"
            )
            self.stdout.write(
                f"📊 Total MongoDB records: {result.get('total_mongo_records', 0)}"
            )
            ...
            if result.get("is_consistent", False):
                ...
                self.stdout.write(
                    f"  - Orphaned MongoDB records: {result.get('orphaned_mongo_count', 0)}"
                )
                self.stdout.write(
                    f"  - Orphaned PostgreSQL records: {result.get('orphaned_pg_count', 0)}"
                )
                ...
                if result.get("orphaned_pg_details"):
                    ...
                    for detail in result["orphaned_pg_details"]:
                        self.stdout.write(
                            f"  - ID {detail['employee_id']}: {detail['employee_name']} "
                ...
                if not dry_run:
                    ...
                    if "BiometricProfile.objects.filter" in command:
                        ...
                        if "update(is_active=False" in command:
                            ...
                            if match:
                                ...
                                if updated:
                                    ...
                                    self.stdout.write(
                                        self.style.WARNING("   ⚠️ No changes made")
                                    )
                        ...
                        if match:
                            employee_id = int(match.group(1))
                            profile, created = (
                                BiometricProfile.objects.update_or_create(
                                    employee_id=employee_id,
                                    defaults={
                                        "is_active": True,
                                        "embeddings_count": 1,
                            ...
                            self.stdout.write(
                                self.style.SUCCESS(f"   ✅ Profile {action}")
                            )
                            fixed_count += 1
                        ...
                        if match:
                            employee_id = int(match.group(1))
                            if mongo_biometric_repository.delete_embeddings(
                                employee_id
                            ):
                                self.stdout.write(
                                    self.style.SUCCESS("   ✅ MongoDB data deleted")
                                )
                                fixed_count += 1
                            else:
                                self.stdout.write(
                                    self.style.WARNING("   ⚠️ No MongoDB data found")
                                )

                    else:
                        self.stdout.write(
                            self.style.WARNING("   ⚠️ Unknown command format - skipped")
                        )
                else:
