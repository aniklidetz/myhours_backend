...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--employee-id',
            type=int,
            help='Specific employee ID to clean up'
        )
        parser.add_argument(
            '--all',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if not employee_ids:
            self.stdout.write(
                self.style.WARNING('No biometric data found to clean up')
            )
            return
    ...
    def process_employee(self, employee_id: int, dry_run: bool, force: bool):
        ...
        with transaction.atomic():
            ...
            if success:
                self.stdout.write(
                    self.style.SUCCESS(f'  Successfully cleaned up employee {employee_id}')
                )
            else:
                raise Exception('delete_biometric returned False')

            # Verify cleanup
            post_status = enhanced_biometric_service.get_employee_biometric_status(employee_id)
