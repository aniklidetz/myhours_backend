...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--employee-id", type=int, help="Sync specific employee ID only"
        )
        parser.add_argument(
            "--create-missing",
            action="store_true",
    ...
    def sync_employee(self, employee_id, create_missing=False):
        ...
        try:
            ...
            self.stdout.write(
                f"Syncing employee {employee_id}: {employee.get_full_name()}"
            )
            ...
            try:
                ...
                self.stdout.write(
                    f"  PostgreSQL profile: {profile.embeddings_count} embeddings, active: {profile.is_active}"
                )
            ...
            if mongo_embeddings:
                ...
                if create_missing and profile.embeddings_count > 0:
                    ...
                    self.stdout.write(
                        f"  WARNING: Employee {employee_id} needs to re-register their face"
                    )
