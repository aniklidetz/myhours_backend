...
class EnhancedBiometricService:
    ...
    def register_biometric(self, employee_id: int, face_encodings: List[Dict]) -> BiometricProfile:
        """
        ...
        with transaction.atomic():
            ...
            try:
                ...
                logger.debug(f"✅ Employee validation passed: {employee.get_full_name()}")
            ...
            try:
                ...
                logger.info(f"✅ PostgreSQL profile {action} for employee {employee_id}")
    ...
    def verify_biometric(self, face_encoding: List[float]) -> Optional[Tuple[int, float]]:
        """
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"✅ Biometric match found: employee_id={employee_id}, confidence={confidence:.3f}")
            else:
    ...
    def delete_biometric(self, employee_id: int) -> bool:
        ...
        try:
            ...
            if mongo_deleted:
                logger.info(f"✅ MongoDB deletion successful for employee {employee_id}")
            else:
                logger.warning(f"⚠️ No MongoDB data found for employee {employee_id}")
        except Exception as e:
            logger.error(f"❌ MongoDB deletion failed for employee {employee_id}: {str(e)}")
            success = False
        ...
        try:
            ...
            if updated:
                ...
                logger.warning(f"⚠️ No PostgreSQL profile found for employee {employee_id}")
        except Exception as e:
            logger.error(f"❌ PostgreSQL update failed for employee {employee_id}: {str(e)}")
            success = False
    ...
    def audit_consistency(self) -> Dict:
        ...
        try:
            ...
            pg_profiles = BiometricProfile.objects.filter(is_active=True).select_related('employee')
            ...
            orphaned_mongo = mongo_employee_ids - pg_employee_ids  # In MongoDB but not in PostgreSQL
            orphaned_pg = pg_employee_ids - mongo_employee_ids      # In PostgreSQL but not in MongoDB
            ...
            if audit_result['is_consistent']:
                logger.info("✅ Biometric data is consistent between MongoDB and PostgreSQL")
            else:
                logger.warning(
    ...
    def get_employee_biometric_status(self, employee_id: int) -> Dict:
        ...
        try:
            ...
            is_consistent = (
                pg_status.get('exists', False) == mongo_status['exists'] and
                (not pg_status.get('exists', False) or 
                 pg_status.get('is_active', False) == (mongo_status['embeddings_count'] > 0))
            )
            ...
            logger.error(f"❌ Failed to get biometric status for employee {employee_id}: {str(e)}")
            return {
                'employee_id': employee_id,
                'is_consistent': False,
                'error': str(e),
