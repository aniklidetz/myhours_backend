...
class MongoBiometricRepository:
    ...
    def _connect(self):
        ...
        try:
            ...
            if self.db is not None:
                ...
                logger.info(f"MongoDB repository connected to '{self.COLLECTION_NAME}' collection")
        ...
        try:
            ...
            if 'employee_id_1' in existing_indexes:
                ...
                if not existing_idx.get('unique', False):
                    logger.info("Dropping non-unique employee_id index to recreate as unique")
    ...
    def save_face_embeddings(self, employee_id: int, embeddings: List[Dict]) -> Optional[str]:
        ...
        try:
            ...
            if saved_doc and saved_doc.get("embeddings"):
                logger.info(f"✅ Verification passed: {len(saved_doc['embeddings'])} embeddings saved")
                return document_id
            else:
                logger.error("❌ Verification failed: Document not found or empty after save")
                return None
            ...
            logger.error(f"❌ Failed to save embeddings for employee {employee_id}: {e}")
            return None
    ...
    def get_face_embeddings(self, employee_id: int) -> Optional[List[Dict]]:
        ...
        try:
            ...
            if document:
                ...
                logger.debug(f"Retrieved {len(embeddings)} embeddings for employee {employee_id}")
                return embeddings
            else:
            ...
            logger.error(f"Failed to retrieve embeddings for employee {employee_id}: {e}")
            return None
    ...
    def get_all_employee_ids(self) -> List[int]:
        ...
        try:
            ...
            logger.debug(f"Found {len(employee_ids)} employees with active biometric data")
            return employee_ids
    ...
    def find_matching_employee(self, face_encoding: List[float], tolerance: float = 0.6) -> Optional[Tuple[int, float]]:
        """
            ...
            for document in cursor:
                ...
                for embedding_data in embeddings:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to calculate distance for employee {employee_id}: {e}")
                        continue
            
            if best_match:
                employee_id, confidence = best_match
                logger.debug(f"Best match found: employee_id={employee_id}, confidence={confidence:.3f}")
            else:
    ...
    def delete_embeddings(self, employee_id: int) -> bool:
        ...
        try:
            ...
            if result.deleted_count > 0:
                ...
                logger.warning(f"⚠️ No embeddings found to delete for employee {employee_id}")
                return False
        ...
        try:
            result = self.collection.update_one(
                {"employee_id": employee_id},
                {
                    "$set": {
                        "is_active": False,
                        "metadata.deactivated_at": datetime.datetime.now(datetime.timezone.utc)
                    }
                }
            )
            ...
            if result.modified_count > 0:
                ...
                logger.warning(f"⚠️ No embeddings found to deactivate for employee {employee_id}")
                return False
                
        except Exception as e:
            logger.error(f"❌ Failed to deactivate embeddings for employee {employee_id}: {e}")
            return False
    ...
    def get_statistics(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "avg_embeddings_per_employee": round(total_embeddings / active_docs, 2) if active_docs > 0 else 0
            }
