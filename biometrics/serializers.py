...
class FaceRegistrationSerializer(serializers.Serializer):
    ...
    employee_id = serializers.IntegerField(help_text="ID of the employee to register")
    ...
    def validate_image(self, value):
        ...
        if value.startswith("data:image"):
            ...
            # Validate image type
            if not any(
                img_type in header for img_type in ["jpeg", "jpg", "png", "webp"]
            ):
                raise serializers.ValidationError(
        ...
        try:
            ...
            if not (
                decoded[:2] == b"\xff\xd8"  # JPEG
                or decoded[:8] == b"\x89PNG\r\n\x1a\n"  # PNG
                or decoded[:4] == b"RIFF"
                and decoded[8:12] == b"WEBP"
            ):  # WebP
                raise serializers.ValidationError(
                    "Image data does not appear to be a valid image format"
                )
