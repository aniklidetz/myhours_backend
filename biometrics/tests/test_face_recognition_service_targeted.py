...
class FaceRecognitionServiceInitTest(TestCase):
    ...
    def test_initialization_cascade_empty(self, mock_logger):
        ...
        try:
            ...
            self.assertIsNone(result)
            mock_logger.error.assert_called_with("Face cascade classifier not initialized")
        finally:
...
class FaceRecognitionServiceExtractFeaturesTest(TestCase):
    ...
    def test_extract_face_features_multiple_faces(self, mock_logger, mock_cv2):
        ...
        self.assertIsNotNone(result)
        mock_logger.warning.assert_called_with("Multiple faces detected (2), using the largest one")
    ...
    def test_extract_face_features_cv2_exception(self, mock_logger, mock_cv2):
        ...
        self.assertIsNone(result)
        mock_logger.error.assert_called_with("Error extracting face features: CV2 processing error")
    ...
    def test_extract_face_features_padding_calculation(self, mock_cv2):
        ...
        mock_cascade = Mock()
        mock_cascade.detectMultiScale.return_value = np.array([[0, 0, 20, 20]])  # Face at corner
        FaceRecognitionService.FACE_CASCADE = mock_cascade
...
class FaceRecognitionServiceSaveTest(TestCase):
    ...
    def test_save_employee_face_decode_failure(self, mock_logger, mock_biometric):
        ...
        with patch.object(FaceRecognitionService, 'decode_image', return_value=None):
            ...
            mock_logger.error.assert_called_with("Failed to decode image for registration")
    ...
    def test_save_employee_face_no_face_detected(self, mock_logger, mock_biometric):
        ...
        with patch.object(FaceRecognitionService, 'decode_image', return_value=mock_image):
            with patch.object(FaceRecognitionService, 'extract_face_features', return_value=None):
                result = FaceRecognitionService.save_employee_face(123, "test_image")
                
                self.assertIsNone(result)
                mock_logger.error.assert_called_with("No face detected for registration")
    ...
    def test_save_employee_face_success_new_employee(self, mock_logger, mock_biometric):
        ...
        with patch.object(FaceRecognitionService, 'decode_image', return_value=mock_image):
            with patch.object(FaceRecognitionService, 'extract_face_features', return_value=mock_face_roi):
                result = FaceRecognitionService.save_employee_face(123, "test_image_data")
    ...
    def test_save_employee_face_exception(self, mock_logger, mock_biometric):
        ...
        with patch.object(FaceRecognitionService, 'decode_image', side_effect=Exception("Decode error")):
            result = FaceRecognitionService.save_employee_face(123, "test_image")
            
            self.assertIsNone(result)
            mock_logger.error.assert_called_with("Error saving employee face: Decode error")
...
class FaceRecognitionServiceVerifyTest(TestCase):
    ...
    def test_extract_face_embedding_alias(self):
        ...
        with patch.object(FaceRecognitionService, 'extract_face_features', return_value="result") as mock_extract:
    ...
    def test_verify_face_success(self):
        ...
        with patch.object(FaceRecognitionService, 'recognize_employee', return_value=mock_result):
            result = FaceRecognitionService.verify_face("test_image", 123, threshold=0.8)
            
            self.assertEqual(result, (True, 123, 0.9))

    def test_verify_face_failure_no_match(self):
        """Test face verification when no match found"""
        with patch.object(FaceRecognitionService, 'recognize_employee', return_value=None):
            result = FaceRecognitionService.verify_face("test_image", 123, threshold=0.8)
...
class FaceRecognitionServiceRecognizeTest(TestCase):
    ...
    @patch('biometrics.services.face_recognition_service.BiometricService')
    @patch('biometrics.services.face_recognition_service.logger')
    def test_recognize_employee_success_high_confidence(self, mock_logger, mock_biometric):
        ...
        with patch.object(FaceRecognitionService, 'decode_image', return_value=mock_image):
            with patch.object(FaceRecognitionService, 'extract_face_features', return_value=mock_face_roi):
                ...
                result = FaceRecognitionService.recognize_employee("test_image", threshold=0.5)
    ...
    def test_recognize_employee_low_confidence(self, mock_logger, mock_biometric):
        ...
        with patch.object(FaceRecognitionService, 'decode_image', return_value=mock_image):
            with patch.object(FaceRecognitionService, 'extract_face_features', return_value=mock_face_roi):
                result = FaceRecognitionService.recognize_employee("test_image", threshold=0.95)
    ...
    def test_recognize_employee_exception(self, mock_logger, mock_biometric):
        ...
        with patch.object(FaceRecognitionService, 'decode_image', side_effect=Exception("Recognition error")):
            result = FaceRecognitionService.recognize_employee("test_image")
...
class FaceRecognitionServiceIntegrationTest(TestCase):
    ...
    def test_full_pipeline_no_cv2(self):
        ...
        with patch('biometrics.services.face_recognition_service.cv2', None):
            ...
            result2 = FaceRecognitionService.extract_face_features(np.ones((100, 100, 3)))
    ...
    def test_full_registration_and_recognition_pipeline(self, mock_cv2, mock_biometric):
        ...
        registration_result = FaceRecognitionService.save_employee_face(123, test_image_b64)
        
        self.assertEqual(registration_result, "doc_123")
        
        # Setup for recognition test
        test_encoding = np.ones(10000, dtype=np.float32)
        mock_biometric.get_all_face_encodings.return_value = [(123, test_encoding, "img")]
