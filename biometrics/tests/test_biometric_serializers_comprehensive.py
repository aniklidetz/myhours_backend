...
class FaceRegistrationSerializerTests(BiometricSerializersTestCase):
    ...
    def test_employee_validation_inactive_employee(self):
        ...
        data = {
            'employee_id': self.inactive_employee.id,
            'image': png_data
        }
    ...
    def test_image_validation_parametrized_failures(self):
        ...
        test_cases = [
            ...
            ('data:image/gif;base64,R0lGOD', 'Only JPEG, PNG, and WebP images are supported'),
            ('data:image/png;base64', 'Invalid data URI format'),  # Missing comma
            ('data:image/png;base64,', 'too short'),  # Empty after comma gives short error
        ...
        for invalid_image_data, expected_error in test_cases:
            with self.subTest(image_data=invalid_image_data[:20] + '...', error=expected_error):
                data = {
                    'employee_id': self.employee.id,
                ...
                self.assertTrue(
                    any(expected_error.lower() in error_messages.lower() 
                        for expected_error in [expected_error] if expected_error),
    ...
    def test_image_validation_test_mode_relaxed_limits(self):
        ...
        minimal_png_header = b'\x89PNG\r\n\x1a\n' + b'\x00' * 50  # PNG signature + minimal data
...
class BiometricStatsSerializerTests(BiometricSerializersTestCase):
    ...
    def test_stats_with_error_field(self):
        ...
        serializer = BiometricStatsSerializer(data=data)
        self.assertTrue(serializer.is_valid())
        self.assertEqual(serializer.validated_data['error'], 'MongoDB connection failed')
