...
class IsraeliHolidaysService:
        ...
        if year in cls.INDEPENDENCE_DAY_DATES:
            ...
            try:
                ...
                start_time = timezone.make_aware(
                    datetime.combine(ind_date - timedelta(days=1), datetime.min.time().replace(hour=20))
                )
    ...
    def ensure_independence_day(cls, year):
        ...
        if year in cls.INDEPENDENCE_DAY_DATES:
            ...
            if not Holiday.objects.filter(date=ind_date, name__icontains="Atzmaut").exists():
