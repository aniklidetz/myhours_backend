...
class EnhancedSunriseSunsetService:
    ...
    @classmethod
    def get_times_with_israeli_timezone(cls, date_obj=None, lat=None, lng=None, use_cache=True):
        """
        ...
        try:
            ...
            for key, utc_time_str in results.items():
                try:
                    ...
                    logger.error(f"Error parsing time {key}: {utc_time_str}, error: {e}")
            ...
            logger.error(f"Network error fetching sunrise-sunset data: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error in sunrise-sunset service: {e}")
            return None
    
    @classmethod
    def get_shabbat_times_israeli_timezone(cls, date_obj, lat=None, lng=None, use_cache=True):
        """
        ...
        try:
            ...
            friday_times = cls.get_times_with_israeli_timezone(date_obj, lat, lng, use_cache)
            ...
            try:
                ...
                logger.error(f"Error parsing Friday sunset: {e}")
                fallback_result = cls._get_fallback_shabbat_times(date_obj, saturday)
                if use_cache:
                    cache.set(cache_key, fallback_result, cls.CACHE_TIMEOUT)
                return fallback_result
            ...
            saturday_times = cls.get_times_with_israeli_timezone(saturday, lat, lng, use_cache)
            ...
            if use_cache:
                ...
                logger.info(f"📅 Cached precise Shabbat times for {date_obj} in Israeli timezone")
            
            return result
            
        except Exception as e:
            logger.error(f"Error calculating Israeli Shabbat times for {date_obj}: {e}")
    ...
    def _get_fallback_shabbat_times(cls, friday, saturday):
        ...
        friday_sunset = cls.ISRAEL_TZ.localize(
            datetime.combine(friday, datetime.min.time().replace(
                hour=sunset_hour, minute=sunset_minute
            ))
        )
        
        saturday_sunset = cls.ISRAEL_TZ.localize(
            datetime.combine(saturday, datetime.min.time().replace(
                hour=sunset_hour, minute=sunset_minute + 1
            ))
        )
    ...
    def validate_timezone_consistency(cls):
        ...
        if django_tz != 'Asia/Jerusalem':
            logger.warning(f"Django TIME_ZONE is {django_tz}, but service uses Asia/Jerusalem")
            return False
        
        logger.info("✅ Timezone consistency validated: Django and service both use Asia/Jerusalem")
        return True
