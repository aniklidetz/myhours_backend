...
class SunriseSunsetService:
    ...
    def get_shabbat_times(cls, date_obj, lat=31.7683, lng=35.2137):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error parsing Friday sunset time: {e}")
                fallback_result = {
                    "date": date_obj.isoformat(),
                    "start": (datetime.combine(date_obj, datetime.min.time().replace(hour=18, minute=0))
                            .isoformat()),
    ...
    def is_shabbat_time(cls, check_time, lat=31.7683, lng=35.2137):
        ...
        try:
            ...
            start_time = datetime.fromisoformat(shabbat_times['start'].replace('Z', '+00:00'))
            end_time = datetime.fromisoformat(shabbat_times['end'].replace('Z', '+00:00'))
            ...
            logger.error(f"Error checking Shabbat time: {e}")
            return False