...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--employee-id',
            type=int,
            help='Recalculate only for specific employee ID'
        )
        parser.add_argument(
            '--date',
            type=str,
            help='Recalculate only for specific date (YYYY-MM-DD)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        calculations = DailyPayrollCalculation.objects.filter(**query_filter).select_related(
            'employee', 'employee__salary_info'
        ...
        for calc in calculations:
            try:
                ...
                if not worklog:
                    self.stdout.write(f"⚠️  No worklog found for {calc.employee.get_full_name()} on {calc.work_date}")
                    continue
                ...
                if calculation_type == 'monthly':
                    ...
                    result = service.calculate_daily_bonuses_monthly(worklog, save_to_db=False)
                    ...
                    result = service.calculate_daily_pay_hourly(worklog, save_to_db=False)
                    ...
                    self.stdout.write(f"⚠️  Unsupported calculation type '{calculation_type}' for {calc.employee.get_full_name()}")
                    continue
                ...
                pay_change = (abs(new_base_pay - old_base_pay) > Decimal('0.01') or 
                             abs(new_bonus_pay - old_bonus_pay) > Decimal('0.01'))
                ...
                if sabbath_change or pay_change:
                    if not options['dry_run']:
                        ...
                        if 'sabbath_type' in result:
                            ...
                            calc.calculation_details['sabbath_type'] = result['sabbath_type']
                        
                        calc.save()
                    
                    change_desc = []
                    if sabbath_change:
                        change_desc.append(f"Sabbath: {old_is_sabbath} → {new_is_sabbath}")
                    if pay_change:
                        change_desc.append(f"base ₪{old_base_pay:.2f} → ₪{new_base_pay:.2f}")
                        change_desc.append(f"bonus ₪{old_bonus_pay:.2f} → ₪{new_bonus_pay:.2f}")
                    ...
                    self.stdout.write(f"✓ No change needed for {calc.employee.get_full_name()} - {calc.work_date}")
                
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"Error recalculating {calc.employee.get_full_name()} - {calc.work_date}: {e}")
                )
                errors += 1
                continue
        ...
        if options['dry_run']:
            self.stdout.write(
                self.style.WARNING(f'DRY RUN: Would update {updated_count} records, {errors} errors')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f'Successfully updated {updated_count} records with new Sabbath logic, {errors} errors')
            )