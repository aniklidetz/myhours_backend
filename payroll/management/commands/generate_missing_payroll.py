                        ...
                        for work_log in work_logs:
                            try:
                                if employee.salary_info.calculation_type == 'hourly':
                                    ...
                                    result = service.calculate_daily_bonuses_monthly(work_log, save_to_db=True)
                                
                                if existing_daily:
                                    daily_updated += 1
                                    self.stdout.write(f"  🔄 Updated daily calc for {work_date}")
                                else:
                                    daily_created += 1
                                    self.stdout.write(f"  ➕ Created daily calc for {work_date}")
                                    
                            except Exception as e:
                                self.stdout.write(f"  ❌ Error creating daily calc for {work_date}: {e}")
                                errors += 1
                    else:
            ...
            for summary_year, summary_month in sorted(months_to_process):
                try:
                    ...
                    if existing_monthly and not force:
                        self.stdout.write(f"  ✓ Monthly summary exists for {summary_year}-{summary_month:02d}")
                        continue
                    ...
                    if not dry_run:
                        ...
                        try:
                            ...
                            if existing_monthly:
                                monthly_updated += 1
                                self.stdout.write(f"  🔄 Updated monthly summary for {summary_year}-{summary_month:02d}")
                            else:
                                monthly_created += 1
                                self.stdout.write(f"  ➕ Created monthly summary for {summary_year}-{summary_month:02d}")
                                
                        except Exception as e:
                            self.stdout.write(f"  ❌ Error creating monthly summary for {summary_year}-{summary_month:02d}: {e}")
                            errors += 1
                    else:
                        action = "Update" if existing_monthly else "Create"
                        self.stdout.write(f"  [{action}] Monthly summary for {summary_year}-{summary_month:02d}")

                except Exception as e:
                    self.stdout.write(f"  ❌ Error processing monthly summary for {summary_year}-{summary_month:02d}: {e}")
                    errors += 1
