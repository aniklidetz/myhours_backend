...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write("Updating daily payroll calculations with unified payment structure...")
        ...
        if options['calculation_type'] != 'all':
            query_filter['employee__salary_info__calculation_type'] = options['calculation_type']
        ...
        for calc in calculations:
            try:
                ...
                new_total_gross = result.get('total_gross_pay', result.get('total_pay', Decimal('0')))
        ...
        if options['dry_run']:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Successfully updated {updated_count} daily calculations with unified payment structure, {errors} errors')
            )