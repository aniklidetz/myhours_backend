...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--all", action="store_true", help="Delete all payroll data"
        )
        parser.add_argument(
            "--test-only",
            action="store_true",
    ...
    def cleanup_all_payroll(self, dry_run):
        ...
        self.stdout.write(
            f"📊 Found {compensatory_days.count()} compensatory day records"
        )

        if dry_run:
            self.stdout.write(
                self.style.WARNING("DRY RUN: Would delete all payroll data")
            )
        else:
    ...
    def cleanup_unrealistic_data(self, dry_run):
        ...
        unrealistic_salaries = Salary.objects.filter(
            models.Q(
                base_salary__gt=50000
            )  # Monthly > 50k ILS (unrealistic for most positions)
        ...
        self.stdout.write(
            f"📊 Found {unrealistic_salaries.count()} unrealistic salary records"
        )
        ...
        if unrealistic_salaries.exists():
            ...
            for salary in unrealistic_salaries:
                ...
                self.stdout.write(
                    f"  - {employee_name}: Base={salary.base_salary}, Hourly={salary.hourly_rate}"
                )

        if dry_run:
            self.stdout.write(
                self.style.WARNING("DRY RUN: Would delete unrealistic salary data")
            )
        else:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ Deleted {deleted_count} unrealistic salary records"
                )
            )
