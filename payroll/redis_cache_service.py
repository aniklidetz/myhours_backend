...
from integrations.services.enhanced_sunrise_sunset_service import enhanced_sunrise_sunset_service
...
class PayrollRedisCache:
    ...
    def get_holidays_for_month(self, year: int, month: int) -> Dict[str, Dict]:
        ...
        try:
            ...
            if cached_data:
                ...
                logger.debug(f"📋 Cache HIT: holidays for {year}-{month:02d} ({len(holidays_dict)} holidays)")
            ...
            if holiday.is_shabbat:
                try:
                    ...
                    if shabbat_times:
                        ...
                        logger.debug(f"Enhanced Shabbat {date_str} with precise API times")
                    else:
                        holiday_data['api_enhanced'] = False
                        
                except Exception as e:
                    logger.warning(f"Failed to enhance Shabbat {date_str} with API times: {e}")
                    holiday_data['api_enhanced'] = False
    ...
    def cache_holidays_for_month(self, year: int, month: int, holidays_dict: Dict[str, Dict]):
    ...
    def get_holidays_for_date_range(self, start_date: date, end_date: date) -> Dict[str, Dict]:
        ...
        while current_date <= end_date:
            month_holidays = self.get_holidays_for_month(current_date.year, current_date.month)
        ...
        logger.info(f"📋 Loaded {len(filtered_holidays)} holidays for range {start_date} to {end_date}")
    ...
    def cache_daily_calculation(self, employee_id: int, work_date: date, calculation_data: Dict):
    ...
    def get_monthly_summary(self, employee_id: int, year: int, month: int) -> Optional[Dict]:
        ...
        try:
            cached_data = self.redis_client.get(cache_key)
            if cached_data:
                logger.debug(f"📊 Cache HIT: monthly summary for employee {employee_id} {year}-{month:02d}")
                return json.loads(cached_data)
        except Exception as e:
    ...
    def cache_monthly_summary(self, employee_id: int, year: int, month: int, summary_data: Dict):
        ...
        try:
            ...
            logger.debug(f"📊 Cached monthly summary for employee {employee_id} {year}-{month:02d}")
        except Exception as e:
