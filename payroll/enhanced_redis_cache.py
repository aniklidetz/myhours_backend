...
class EnhancedPayrollCache(PayrollRedisCache):
    ...
    def get_holidays_with_shabbat_times(self, year: int, month: int) -> Dict[str, Dict]:
        ...
        for date_str, holiday_data in holidays_dict.items():
            if holiday_data.get('is_shabbat'):
                ...
                shabbat_times = enhanced_sunrise_sunset_service.get_shabbat_times_israeli_timezone(work_date)
                ...
                if shabbat_times:
                    ...
                    holiday_data.update({
                        ...
                        'calculation_method': shabbat_times.get('calculation_method', 'api_precise')
                    })
    ...
    def cache_shabbat_times_for_month(self, year: int, month: int):
        ...
        try:
            ...
            for friday in fridays:
                shabbat_times = enhanced_sunrise_sunset_service.get_shabbat_times_israeli_timezone(friday)
                if shabbat_times:
            ...
            logger.info(f"📅 Cached enhanced Shabbat times for {len(fridays)} Fridays in {year}-{month:02d}")
    ...
    def is_work_during_shabbat(self, work_start, work_end, work_date) -> Dict[str, Any]:
        ...
        Returns:
            Dict with overlap info and rates to apply
        """
