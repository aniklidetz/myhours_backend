...
class EnhancedEarningsSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        """
        if (
            not hasattr(instance, "employee")
            or not hasattr(instance, "year")
            or not hasattr(instance, "month")
        ):
        ...
        return {
            ...
            "summary": {
                ...
                "worked_days": len(
                    [calc for calc in result.get("daily_calculations", [])]
                ),
            ...
            "compensatory_days": self._build_compensatory_breakdown(
                comp_balance, comp_days_this_period
            ),
        ...
        for comp_day in current_period_days:
            ...
            if comp_day.reason == "holiday":
                ...
                holiday = Holiday.objects.filter(
                    date=comp_day.date_earned, is_holiday=True
                ).first()
                if holiday:
                ...
                sabbath = Holiday.objects.filter(
                    date=comp_day.date_earned, is_shabbat=True
                ).first()
    ...
    def _build_daily_breakdown(self, daily_calculations):
        ...
        for calc in daily_calculations:
            ...
            if calc["is_holiday"]:
                ...
                if calc["breakdown"]:
                    day_info["breakdown"]["regular"] = float(
                        calc["breakdown"].get("regular_hours", 0)
                    )
    ...
    def _build_attendance_info(self, result):
        ...
        return {
            ...
            "days_missed": max(
                0, result.get("total_working_days", 0) - result.get("worked_days", 0)
            ),
            "attendance_rate": (
                round(
                    (result.get("worked_days", 0) / result.get("total_working_days", 1))
                    * 100,
                    2,
                )
                if result.get("total_working_days", 0) > 0
                else 0
            ),
        }
