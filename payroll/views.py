        ...
        if employee_profile:
            ...
            if user_role in ["admin", "accountant"]:
                ...
                try:
                    ...
                    logger.info(
                        "Employee view",
                        extra=safe_log_employee(employee, "payroll_view"),
                    )
                except Salary.DoesNotExist:
                    logger.warning(
                        f"Employee {request.user.username} has no salary configuration"
                    )
...
def payroll_analytics(request):
    ...
    try:
        ...
        return Response({"year": year, "monthly_statistics": list(monthly_stats)})
    ...
    try:
        ...
        if employee_id:
            if not (
                ...
                and request.user.employee_profile.role in ["accountant", "admin"]
            ):
                return Response(
                    {"error": "Permission denied"}, status=status.HTTP_403_FORBIDDEN
                )
        ...
        if work_date:
            ...
            calculations = calculations.filter(
                work_date__year=current_date.year, work_date__month=current_date.month
            )
    ...
    try:
        ...
        try:
            summary = MonthlyPayrollSummary.objects.get(
                employee=target_employee, year=year, month=month
            )
            ...
            daily_calculations = DailyPayrollCalculation.objects.filter(
                employee=target_employee, work_date__year=year, work_date__month=month
            ).order_by("work_date")
...
def recalculate_payroll(request):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Payroll recalculated for {target_employee.get_full_name()} {year}-{month:02d}"
            )
            ...
            return Response(
                {"error": "Payroll calculation failed", "details": str(calc_error)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    try:
        ...
        analytics = {
            ...
            "total_gross_payroll": float(
                summaries.aggregate(Sum("total_gross_pay"))["total_gross_pay__sum"] or 0
            ),
            "average_salary": float(
                summaries.aggregate(Avg("total_gross_pay"))["total_gross_pay__avg"] or 0
            ),
            "total_hours_worked": float(
                summaries.aggregate(Sum("total_hours"))["total_hours__sum"] or 0
            ),
            "total_overtime_hours": float(
                summaries.aggregate(Sum("overtime_hours"))["overtime_hours__sum"] or 0
            ),
            "total_holiday_hours": float(
                summaries.aggregate(Sum("holiday_hours"))["holiday_hours__sum"] or 0
            ),
            "total_sabbath_hours": float(
                summaries.aggregate(Sum("sabbath_hours"))["sabbath_hours__sum"] or 0
            ),
            "employees_with_overtime": summaries.filter(overtime_hours__gt=0).count(),
            "employees_with_holiday_work": summaries.filter(
                holiday_hours__gt=0
            ).count(),
            "employees_with_sabbath_work": summaries.filter(
                sabbath_hours__gt=0
            ).count(),
            "total_compensatory_days": summaries.aggregate(
                Sum("compensatory_days_earned")
            )["compensatory_days_earned__sum"]
            or 0,
            "employees_with_violations": summaries.filter(
                legal_violations_count__gt=0
            ).count(),
