...
class OptimizedPayrollService:
    ...
    def calculate_bulk_payroll(self, employees_queryset, year: int, month: int) -> List[Dict]:
        """
        ...
        """
        logger.info(f"🚀 Starting optimized bulk payroll calculation for {year}-{month:02d}")
        ...
        try:
            holidays_cache = enhanced_payroll_cache.get_holidays_with_shabbat_times(year, month)
            
            if holidays_cache:
                self.api_usage['cache_hits'] += 1
                logger.info(f"📋 Loaded {len(holidays_cache)} holidays with precise Shabbat times from cache")
            else:
        ...
        try:
            ...
            logger.info(f"📊 Found {len(existing_summaries)} existing monthly summaries")
        except Exception as e:
        ...
        for employee in employees:
            try:
                ...
                if employee.id in existing_summaries:
                    ...
                    result = self._convert_summary_to_result(employee, summary, year, month)
                    results.append(result)
                    logger.debug(f"✅ Used cached summary for {employee.get_full_name()}")
                    continue
        ...
        logger.info(f"✅ Bulk payroll calculation completed. Processed {len(results)} employees")
        logger.info(f"📊 API Usage: {self.api_usage}")
        
        return results
    
    def _calculate_single_employee_optimized(self, employee, year: int, month: int, holidays_cache: Dict) -> Dict:
        """
        ...
        try:
            ...
            if not hasattr(employee, 'salary_info') or not employee.salary_info:
                return self._create_error_result(employee, year, month, "Employee has no salary configuration")
            ...
            if not work_logs:
                logger.debug(f"No work logs found for {employee.get_full_name()} in {year}-{month:02d}")
                return {
                    "id": employee.id,
                    "employee": {
                        "id": employee.id,
    ...
    def _convert_summary_to_result(self, employee, summary: MonthlyPayrollSummary, year: int, month: int) -> Dict:
        """
    ...
    def _create_error_result(self, employee, year: int, month: int, error_message: str) -> Dict:
        """
        Create error result
        """
        return {
