...
class ShabbatTimes(TypedDict):
    ...
    coordinates: Dict[
        str, float
    ]  # {"lat": 31.7683, "lng": 35.2137} - Jerusalem default
...
def validate_shabbat_times(result: dict) -> ShabbatTimes:
    ...
    if result.get("timezone") != "Asia/Jerusalem":
        raise ValidationError(
            f"Invalid timezone: {result.get('timezone')}. Must be 'Asia/Jerusalem'"
        )
    ...
    if result.get("calculation_method") not in valid_methods:
        raise ValidationError(
            f"Invalid calculation_method: {result.get('calculation_method')}. Must be one of {valid_methods}"
        )
        ...
        for field in [
            ...
            if not isinstance(result[field], Decimal):
                try:
                    ...
                    raise ValidationError(
                        f"Cannot convert {field} to Decimal: {result[field]} - {e}"
                    )
...
def create_fallback_shabbat_times(
    ...
    friday_sunset = (
        f"{friday.isoformat()}T{sunset_hour:02d}:{sunset_minute:02d}:00+02:00"
    )
    saturday_sunset = (
        f"{saturday.isoformat()}T{sunset_hour:02d}:{sunset_minute+1:02d}:00+02:00"
    )
...
def create_empty_payroll_result(
    employee_id: int, strategy: str = "unknown", currency: str = "ILS"
) -> PayrollResult:
    """
