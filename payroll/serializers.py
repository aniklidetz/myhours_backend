...
class SalarySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Salary
        fields = [
            'id', 'employee', 'employee_name', 'base_salary', 'hourly_rate',
            'calculation_type', 'project_start_date', 'project_end_date',
            'project_completed', 'currency', 'calculated_salary',
    ...
    def validate_date_used(self, value):
        ...
        if value:
            ...
            if value > timezone.now().date():
                raise serializers.ValidationError(
                    "Used date cannot be in the future"
                )
        return value