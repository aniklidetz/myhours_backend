...
PayrollService for accurate Sabbath overtime calculations.
"""
...
class ShiftSplittingTest(PayrollTestMixin, TestCase):
    ...
    def test_split_overtime_calculation(self):
        ...
        total_hours = Decimal("10.6")
        sabbath_hours = Decimal("2.0")
        breakdown = ShiftSplitter.calculate_split_overtime(total_hours, sabbath_hours)
        # Should have 8.6 hours before Sabbath, 2 during
        before_sabbath_hours = total_hours - sabbath_hours  # 8.6 hours
        # Before Sabbath: 8.6 regular hours norm, so 8.6 regular + 0 overtime
        self.assertEqual(breakdown["regular_hours"], ISRAELI_DAILY_NORM_HOURS)
    ...
    def test_payment_calculation_for_split_shift(self):
        ...
        monthly_hourly = Decimal("100.00")
        payment = ShiftSplitter.calculate_payment_for_split_shift(
            breakdown, monthly_hourly
        )
        # Calculate expected payments
        expected_regular = ISRAELI_DAILY_NORM_HOURS * monthly_hourly  # 860
        expected_sabbath_regular = (
            Decimal("0.60") * monthly_hourly * Decimal("1.5")
        )  # 90 (150%)
        expected_sabbath_ot1 = (
            Decimal("1.40") * monthly_hourly * Decimal("1.75")
        )  # 245 (175%)
        expected_total = (
            expected_regular + expected_sabbath_regular + expected_sabbath_ot1
        )  # 1135
        self.assertEqual(payment["base_regular_pay"], expected_regular)
        ...
        self.assertEqual(payment["total_salary"], expected_total)
    @patch.object(SunriseSunsetService, "get_shabbat_times")
    def test_friday_shift_spanning_sabbath_hourly_employee(self, mock_sabbath_times):
        ...
        # Should have worked around 6 hours
        total_hours = result.get("total_hours", 0)
        self.assertAlmostEqual(float(total_hours), 6.0, places=1)
        # Payment should be more than simple 6 × 120 due to Sabbath premiums
        simple_pay = 6 * 120  # 720
        self.assertGreater(result.get("total_salary", 0), simple_pay)
    @patch.object(SunriseSunsetService, "get_shabbat_times")
    def test_friday_shift_spanning_sabbath_monthly_employee(self, mock_sabbath_times):
        ...
        # Calculate with API integration (fast_mode=False)
        context = make_context(self.monthly_employee, 2025, 7, fast_mode=False)
        result = self.payroll_service.calculate(context, CalculationStrategy.ENHANCED)
        # Verify calculation completed successfully
        self.assertGreater(result.get("total_salary", 0), 0)
        self.assertGreater(result.get("total_hours", 0), 0)
        # Should have worked around 9 hours
        total_hours = result.get("total_hours", 0)
        self.assertAlmostEqual(float(total_hours), 9.0, places=1)
        # Monthly employee should get reasonable compensation
        monthly_hourly_rate = self.monthly_salary.base_salary / MONTHLY_NORM_HOURS  # ≈ 109.89
        expected_min_pay = Decimal("9.0") * monthly_hourly_rate  # Base proportional salary
        self.assertGreater(result.get("total_salary", 0), expected_min_pay)
    ...
    def test_database_integration_saves_split_shift_data(self):
        ...
        self.assertGreater(result.get("total_salary", 0), 0)
        self.assertGreater(result.get("total_hours", 0), 0)
        # Should have worked around 4 hours
        total_hours = result.get("total_hours", 0)
        self.assertAlmostEqual(float(total_hours), 4.0, places=1)
        # Payment should include Sabbath premiums
        proportional_monthly = 4 * 120  # 480
        self.assertGreater(result.get("total_salary", 0), proportional_monthly)
class ShiftSplitterEdgeCasesTest(PayrollTestMixin, TestCase):
    ...
    def test_overtime_calculation_with_large_overtime_before_sabbath(self):
        ...
        self.assertEqual(breakdown["regular_hours"], ISRAELI_DAILY_NORM_HOURS)  # Daily norm
        self.assertEqual(
    ...
    def test_overtime_calculation_with_no_sabbath_overtime_available(self):
        ...
        self.assertEqual(breakdown["regular_hours"], ISRAELI_DAILY_NORM_HOURS)
        self.assertEqual(breakdown["overtime_before_sabbath_1"], Decimal("2.00"))
        self.assertEqual(
    ...
    def test_payment_calculation_with_all_overtime_categories(self):
        ...
        self.assertEqual(payment["base_regular_pay"], Decimal("860.00"))  # 8.6 * 100
        self.assertEqual(
