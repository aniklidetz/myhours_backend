...
class CriticalPointsAlgorithmTestCase(TestCase):
    ...
    def setUp(self):
        ...
        Holiday.objects.get_or_create(
            date=date(2025, 1, 17), defaults={"name": "Shabbat", "is_shabbat": True}
        )  # Friday evening
        Holiday.objects.get_or_create(
            date=date(2025, 1, 18), defaults={"name": "Shabbat", "is_shabbat": True}
        )  # Saturday
    ...
    def _create_shift(self, employee, start_str, end_str):
        ...
        start_dt = timezone.make_aware(
            datetime.strptime(
                f"{day_mapping[start_parts[0]]} {start_parts[1]}", "%Y-%m-%d %H:%M"
            )
        )
        end_dt = timezone.make_aware(
            datetime.strptime(
                f"{day_mapping[end_parts[0]]} {end_parts[1]}", "%Y-%m-%d %H:%M"
            )
        )
        ...
        # Create worklog
        WorkLog.objects.create(employee=employee, check_in=start_dt, check_out=end_dt)
    ...
    def test_normative_hours_conversion(self):
        ...
        WorkLog.objects.create(
            employee=self.hourly_employee, check_in=thursday_dt, check_out=end_dt
        )
...
class HourlyVsMonthlyComparisonTestCase(TestCase):
    ...
    def _create_shift_for_both(self, start_dt, end_dt):
        ...
        for employee_type, employee in [
            ("hourly", self.hourly_employee),
            ("monthly", self.monthly_employee),
        ]:
            WorkLog.objects.filter(employee=employee).delete()
            WorkLog.objects.create(
                employee=employee, check_in=start_dt, check_out=end_dt
            )
