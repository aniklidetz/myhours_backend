...
class TestEnhancedPayrollStrategy:
    ...
    def mock_work_logs_regular(self):
        ...
        for day in range(1, 21):  # 20 work days
            log = Mock()
            log.check_in = timezone.make_aware(
                datetime(2025, 1, day, 9, 0)
            )  # 9 AM start
            ...
            log.check_out = log.check_in + timedelta(
    ...
    def mock_work_logs_with_specials(self):
        ...
        for day in [1, 2, 3, 6, 7, 8, 9, 10]:
            ...
            log.check_out = log.check_in + timedelta(
                hours=float(ISRAELI_DAILY_NORM_HOURS), minutes=60
            )  # Add check_out
        ...
        holiday_log.check_out = holiday_log.check_in + timedelta(
            hours=6, minutes=30
        )  # Add check_out
    ...
    def test_get_employee_with_relations(
        self, mock_objects, strategy, mock_hourly_employee
    ):
    ...
    def test_get_holidays_enhanced_cache_miss(
        self, mock_holiday_objects, mock_cache, strategy
    ):
    ...
    def test_validate_legal_compliance_normal_hours(
        self, strategy, mock_work_logs_regular
    ):
        """Test legal compliance validation with normal work hours"""
        # Regular 8-hour logs should pass validation
        normal_logs = [
            log
            for log in mock_work_logs_regular
            if log.get_total_hours() <= ISRAELI_DAILY_NORM_HOURS
        ]
    ...
    def test_validate_legal_compliance_excessive_hours(self, strategy):
        ...
        excessive_log.get_total_hours.return_value = Decimal(
            "14.0"
        )  # Exceeds 12 hour limit
    ...
    def test_calculate_hourly_employee_flow(
        self,
        mock_calc_hourly,
        mock_get_logs,
        mock_get_holidays,
    ...
    def test_hourly_employee_overtime_calculation(
        self, strategy, mock_hourly_employee, mock_work_logs_regular
    ):
        ...
        with patch.object(strategy, "_get_holidays_enhanced", return_value={}):
            with patch.object(
                ...
                assert (
                    result["metadata"]["calculation_strategy"]
                    == "enhanced_critical_points"
                )
    ...
    def test_monthly_employee_bonus_calculation(self, strategy, mock_monthly_employee):
        ...
        for day in range(1, 6):  # 5 days
            ...
            log.get_total_hours.return_value = Decimal(
                "10.0"
            )  # 1.4 hours over 8.6 norm
            log.employee_id = 456
        ...
        with patch.object(strategy, "_get_holidays_enhanced", return_value={}):
            ...
            assert (
                result["total_salary"] > proportional_base
            )  # Should include overtime bonuses
            assert (
                result["total_salary"] < monthly_salary
            )  # But less than full monthly salary
...
class TestEnhancedStrategyIntegration:
    ...
    def test_holiday_work_premium_calculation(self):
        ...
        holidays = {date(2025, 1, 1): {"name": "New Year", "is_paid": True}}

        with patch.object(
            strategy, "_calculate_night_shift_hours", return_value=Decimal("0.0")
        ):
            result = strategy._calculate_hourly_employee(
                employee, salary, [holiday_log], holidays
            )

            # Verify holiday premium (150% rate)
            assert result["holiday_hours"] == ISRAELI_DAILY_NORM_HOURS
            expected_holiday_pay = (
                ISRAELI_DAILY_NORM_HOURS * Decimal("40.0") * Decimal("1.5")
            )  # 480.0
    ...
    def test_sabbath_work_premium_calculation(self):
        ...
        with patch.object(
            ...
            expected_sabbath_pay = (
                Decimal("6.5") * Decimal("30.0") * Decimal("1.5")
            )  # 292.5
