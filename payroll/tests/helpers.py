...
def create_mock_shabbat_times(friday_date):
    ...
    if isinstance(friday_date, datetime):
        ...
        friday_date = timezone.make_aware(
            datetime.combine(friday_date, datetime.min.time())
        )
...
class PayrollTestMixin:
    ...
    def assert_salary_structure(
        self, result: Dict[str, Any], expected_keys: Optional[set] = None
    ):
    ...
    def assert_decimal_field(
        self, result: Dict[str, Any], field_name: str, expected_value: Decimal
    ):
        ...
        self.assertIsInstance(
            actual, Decimal, f"{field_name} should be Decimal, got {type(actual)}"
        )
        self.assertEqual(actual, expected_value, f"{field_name} value mismatch")

    def assert_overtime_calculation(
        self, result: Dict[str, Any], base_hours: Decimal, overtime_hours: Decimal
    ):
        ...
        self.assertEqual(
            total_hours, expected_total, "Total hours calculation incorrect"
        )
