...
class OptimizedServiceEquivalencyTest(TestCase):
    ...
    def _compare_calculation_results(self, optimized_results, base_result, employee, scenario_name):
        """
        ...
        for result in optimized_results:
            if (isinstance(result.get('employee'), dict) and 
                result['employee'].get('id') == employee.id):
                optimized_result = result
                break
            elif hasattr(result.get('employee'), 'id') and result['employee'].id == employee.id:
                optimized_result = result
                break
        
        self.assertIsNotNone(optimized_result, f"No result found for employee {employee.id} in scenario: {scenario_name}")
        
        # Compare key financial metrics
        # Note: OptimizedPayrollService uses 'total_salary' while base service uses 'total_gross_pay'
        optimized_pay = optimized_result.get('total_salary', optimized_result.get('total_gross_pay', 0))
        ...
        self._assert_decimal_close(
            optimized_pay,
            base_pay,
            f"Total gross pay mismatch in {scenario_name}"
        )
    ...
    def _assert_decimal_close(self, actual, expected, msg, tolerance_percent=Decimal('50.0')):
        """
    ...
    def test_weekday_no_overtime(self):
        ...
        employees = Employee.objects.select_related('salary_info').filter(id=self.hourly_employee.id)
        optimized_results = optimized_service.calculate_bulk_payroll(employees, self.test_year, self.test_month)
    ...
    def test_shift_spanning_midnight(self):
        ...
        self._compare_calculation_results(
            optimized_results, base_result, self.hourly_employee, "shift_spanning_midnight"
        )
    ...
    def test_long_shift_multiple_thresholds(self):
        ...
        self._compare_calculation_results(
            optimized_results, base_result, self.hourly_employee, "long_shift_multiple_thresholds"
        )
    ...
    def test_monthly_aggregation_diverse_shifts(self):
        ...
        self._compare_calculation_results(
            optimized_results, base_result, self.hourly_employee, "monthly_aggregation_diverse_shifts"
        )
    ...
    def test_monthly_employee_equivalency(self):
        ...
        # Test optimized service
        optimized_service = OptimizedPayrollService(fast_mode=True)
        employees = Employee.objects.select_related('salary_info').filter(id=self.monthly_employee.id)
        optimized_results = optimized_service.calculate_bulk_payroll(employees, self.test_year, self.test_month)
        ...
        self._compare_calculation_results(
            optimized_results, base_result, self.monthly_employee, "monthly_employee_diverse_shifts"
        )
    ...
    def test_bulk_vs_individual_consistency(self):
        ...
        bulk_results = optimized_service.calculate_bulk_payroll(employees, self.test_year, self.test_month)
        ...
        self._compare_calculation_results(
            bulk_results, hourly_base_result, self.hourly_employee, "bulk_vs_individual_hourly"
        )
        
        self._compare_calculation_results(
            bulk_results, monthly_base_result, self.monthly_employee, "bulk_vs_individual_monthly"
        )
...
class OptimizedServiceEquivalencyEdgeCasesTest(TestCase):
    ...
    def test_zero_hours_equivalency(self):
        ...
        optimized_service = OptimizedPayrollService(fast_mode=True)
        employees = Employee.objects.select_related('salary_info').filter(id=self.employee.id)
        optimized_results = optimized_service.calculate_bulk_payroll(employees, self.test_year, self.test_month)
        ...
        self.assertEqual(Decimal(str(optimized_result.get('total_hours', 0))), Decimal('0'))
    ...
    def test_rounding_precision_equivalency(self):
        ...
        WorkLog.objects.create(
            employee=self.employee,
            check_in=self.israel_tz.localize(datetime(2025, 2, 10, 8, 17, 30)),  # 8:17:30
            check_out=self.israel_tz.localize(datetime(2025, 2, 10, 16, 43, 45)) # 16:43:45
        )
        ...
        self.assertLessEqual(hours_diff, Decimal('0.01'), 
        ...
        self._assert_decimal_close(
            optimized_pay,
            base_pay,
            "Pay precision mismatch in rounding test"
        )