...
class CompensatoryDayTest(TestCase):
    ...
    def test_create_compensatory_day_holiday_work(self):
        ...
        success, comp_day = service.create_compensatory_day(
            work_date, reason, work_hours
        )
    ...
    def test_create_compensatory_day_duplicate_prevention(self):
        ...
        comp_days_count = CompensatoryDay.objects.filter(
            employee=self.employee, date_earned=work_date
        ).count()
...
class OvertimeCalculationTest(TestCase):
    ...
    def test_calculate_overtime_pay_hourly_basic(self):
        ...
        expected_overtime_125 = (
            min(overtime_hours, Decimal("2")) * base_rate * Decimal("1.25")
        )  # 2 * 50 * 1.25 = 125
        expected_overtime_150 = (
            max(overtime_hours - Decimal("2"), Decimal("0"))
            * base_rate
            * Decimal("1.50")
        )  # 1 * 50 * 1.50 = 75
    ...
    def test_calculate_overtime_pay_hourly_holiday(self):
        ...
        self.assertGreater(
            result["total_pay"], regular_hours * base_rate
        )  # Should be more than regular pay
...
class MonthlyEmployeeBonusTest(TestCase):
    ...
    def test_calculate_daily_bonuses_monthly_basic(self):
        ...
        work_log_mock = Mock()
        work_log_mock.get_duration_decimal.return_value = Decimal(
            "8.0"
        )  # 8 hours worked (regular)
        ...
        with (
            patch.object(service, "is_night_shift", return_value=(False, Decimal("0"))),
            patch.object(
                service, "is_sabbath_work_precise", return_value=(False, None, None)
            ),
            patch.object(service, "is_holiday_work_enhanced", return_value=None),
        ):

            result = service.calculate_daily_bonuses_monthly(
                work_log_mock, save_to_db=False
            )

        # Monthly employee should have minimal bonuses for regular work
        expected_base_pay = Decimal("8.0") * (
            service.salary.base_salary / service.MONTHLY_WORK_HOURS
        )
        self.assertEqual(result["base_pay"], expected_base_pay)
        self.assertEqual(
            result["bonus_pay"], Decimal("0")
        )  # No bonus pay for regular hours
    ...
    def test_calculate_daily_bonuses_monthly_overtime(self):
        ...
        expected_base_pay = Decimal("10.0") * (
            service.salary.base_salary / service.MONTHLY_WORK_HOURS
        )
    ...
    def test_calculate_daily_bonuses_monthly_holiday(self):
        ...
        work_log_mock.check_in = timezone.make_aware(
        ...
        with (
            ...
            patch.object(
                service, "is_holiday_work_enhanced", return_value=mock_holiday
            ),
            patch.object(
                service, "create_compensatory_day", return_value=(True, Mock())
            ),
        ):
        ...
        expected_holiday_bonus = (
            expected_base_pay * service.SPECIAL_DAY_BONUS
        )  # 50% bonus
