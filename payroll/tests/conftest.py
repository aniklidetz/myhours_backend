...
def make_context(
    ...
    **kwargs
    ...
    if employee_type is None:
        if (hasattr(employee, 'salaries') and 
            employee.salaries.filter(is_active=True, calculation_type='hourly').exists()):
...
def assert_result_structure(result: dict, expected_keys: Optional[set] = None):
    ...
    if expected_keys is None:
        expected_keys = {
            'total_salary', 'total_hours', 'base_pay', 'bonus_pay',
            'breakdown', 'is_sabbath', 'calculation_method'
        }
