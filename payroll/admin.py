...
class SalaryAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not settings.FEATURE_FLAGS.get("ENABLE_PROJECT_PAYROLL", False):
            calculation_choices = [
                choice
                for choice in self.fields["calculation_type"].choices
                if choice[0] != "project"
            ]
            ...
            if (
                ...
                and self.instance.calculation_type == "project"
            ):
...
class DailyPayrollCalculationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Employee & Date", {"fields": ("employee", "work_date")}),
        (
            "Hours Breakdown",
            {
                "fields": (
        ...
        (
            ...
            {"fields": (("base_pay", "bonus_pay"), "total_gross_pay")},
        ),
        (
...
class MonthlyPayrollSummaryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Employee & Period", {"fields": ("employee", ("year", "month"))}),
        (
            "Hours Summary",
            {
                "fields": (
        ...
        ("Work Statistics", {"fields": (("worked_days", "compensatory_days_earned"))}),
        (
            "System Information",
            {
                "fields": ("calculation_details", ("calculation_date", "last_updated")),
                "classes": ("collapse",),
            },
        ),
    )
